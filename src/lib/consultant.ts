// AI LABS INC. — Autonomous AI Consultant runtime helpers.
...
function buildProjectFiles(spec: DesignSpec): { name: string; content: string }[] {
  ...
  const catalog =
    `// Industry catalog generated by the AI Labs Context-Aware Engine.\n` +
  ...
  const appJson = JSON.stringify(
    {
      expo: {
        ...
        android: { package: `inc.ailabs.${slug.replace(/-/g, '')}` },
      },
    },
    null,
    2,
  ...
  const readme = [
    ...
    `${spec.description}`,
    ``,
    `Generated by the **AI Labs Inc. Context-Aware Engine** as a real Expo /`,
    ...
    `> so AI Labs exports the honest, buildable source instead of a broken link.`,
    ``,
  ].join('\n')
